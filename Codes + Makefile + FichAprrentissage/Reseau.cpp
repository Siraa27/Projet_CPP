...
{
  couches.push_back(new CoucheCachee(nbNeuronesParCouches[0], nbNeuroneEntrees));
  Matrice mat = couches[0]->getLiaisonEntrees();
  mat.initAleatoire();
  ...
  if(abs(choixPoids)<= 0.0001 ){
    for(int i=1;i<nbCouchesCachees;i++){
      couches.push_back(new CoucheCachee(nbNeuronesParCouches[i], nbNeuronesParCouches[i-1]));
...
CoucheSorties Reseau::getSorties() const{
  return sorties;
}

int Reseau::getNbNeuronesEntree() const{
...
void Reseau::calcGradErr(const CoucheSorties sAttendues){
  int i, j;
...
void Reseau::Remplissage(vector<CoucheEntrees*>& x, vector<CoucheSorties*>& y, string nomFic)
{
    ...
    {
        x.push_back(new CoucheEntrees(nbNeurones));
        y.push_back(new CoucheSorties(nbColonnes-nbNeurones, couches[nbCouchesCachees-1]->getNbNeurones()));
    }
